...
import ListDataSource from "@spreadsheet_oca/vendor/spreadsheet/list/list_data_source";
import PivotDataSource from "@spreadsheet_oca/vendor/spreadsheet/pivot/pivot_data_source";
...
export class ActionSpreadsheetOca extends Component {
    setup() {
        ...
        useSubEnv({
            ...
            // The owl 1 client action calls it back when the action is left
    ...
    /**
     * Called by the owl 1 client action wrapper before leaving the action.
     */
    beforeLeave() {
        if (this.renderer) {
            this.renderer.onSpreadsheetSaved();
        }
    }
