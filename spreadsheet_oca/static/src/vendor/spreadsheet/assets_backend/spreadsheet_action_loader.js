...
import {getBundle, loadBundle} from "@spreadsheet_oca/vendor/web/assets";
...
export async function loadSpreadsheetAction(env, actionName, actionLazyLoader) {
    const desc = await getBundle(SPREADSHEET_BUNDLE, env.services.rpc);
    await loadBundle(desc);

    if (actionRegistry.get(actionName) === actionLazyLoader) {
        // At this point, the real spreadsheet client action should be loaded and
        // have replaced this function in the action registry. If it's not the
        // case, it probably means that there was a crash in the bundle (e.g.
        // syntax error). In this case, this action would remain in the registry,
